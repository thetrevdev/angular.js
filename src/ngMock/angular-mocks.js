  ...
  self.defer.flush = function(delay) {
    if (angular.isDefined(delay)) {
      ...
      if (self.deferredFns.length) {
        ...
        throw new NgError(29, 'No deferred tasks to be flushed');
      }
    }
  ...
  this.mode = function(mode) {
    switch(mode) {
      ...
      case 'log':
        ...
        break;
      default:
        throw new NgError(30, "Unknown mode '{0}', only 'log'/'rethrow' modes are allowed!", mode);
    }
  };
  ...
  this.$get = function () {
    ...
    $log.assertEmpty = function() {
      ...
      if (errors.length) {
        ...
        errors.push('');
        throw new NgError(31, '{0}', errors.join('\n---------\n'));
      }
    };
  ...
  angular.mock.TzDate = function (offset, timestamp) {
    ...
    angular.forEach(unimplementedMethods, function(methodName) {
      self[methodName] = function() {
        throw new NgError(32, "Method '{0}' is not implemented in the TzDate mock", methodName);
      };
    });
...
function createHttpBackendMock($delegate, $browser) {
  ...
  function $httpBackend(method, url, data, callback, headers) {
    ...
    while ((definition = definitions[++i])) {
      if (definition.match(method, url, data, headers || {})) {
        ...
        } else throw new NgError(35, 'No response defined !');
        return;
      }
    }
  ...
  $httpBackend.flush = function(count) {
    if (!responses.length) throw new NgError(38, 'No pending request to flush !');

    if (angular.isDefined(count)) {
      while (count--) {
        if (!responses.length) throw new NgError(39, 'No more pending request to flush !');
        responses.shift()();
      }
    } else {
  ...
  $httpBackend.verifyNoOutstandingExpectation = function() {
    if (expectations.length) {
      throw new NgError(40, 'Unsatisfied requests: {0}', expectations.join(', '));
    }
  };
  ...
  $httpBackend.verifyNoOutstandingRequest = function() {
    if (responses.length) {
      throw new NgError(41, 'Unflushed requests: {0}', responses.length);
    }
  };
...
window.jasmine && (function(window) {
  ...
  window.module = angular.mock.module = function() {
    ...
    function workFn() {
      var spec = getCurrentSpec();
      if (spec.$injector) {
        throw new NgError(42, 'Injector already created, can not register a module!');
      } else {
